...
from app.api.v1.items_api import create_item_api, create_stock_history_api, delete_item_api, get_item_api, get_items_api, get_stock_by_item_id_api, get_stock_history_by_item_id_api, update_item_api, update_stock_api
...
@router.get("/{item_id}/stock-history", response_model=SuccessResponse)
def get_stock_history_by_item_id(item_id: int, db: Session = Depends(get_db)):
  return get_stock_history_by_item_id_api(item_id, db)

@router.post("/{item_id}/stock-history", response_model=SuccessResponse)
def create_stock_history(item_id: int, request: StockHistoryRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
  return create_stock_history_api(item_id, request, db, current_user)